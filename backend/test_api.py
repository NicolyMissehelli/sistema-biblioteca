...
def test_login_and_basic_library_flow():
    with TestClient(app) as client:
        login = client.post("/auth/login", data={"username": "admin@test.local", "password": "senha123"})
        ...
        rlogin = client.post("/auth/login", data={"username": "leitor@test.local", "password": "senha123"})
