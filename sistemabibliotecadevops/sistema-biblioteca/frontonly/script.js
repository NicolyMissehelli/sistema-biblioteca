...
const api = async (endpoint, options = {}) => {
  try {
    const token = localStorage.getItem("novarisToken");
    ...
    if (!response.ok) throw new Error(data.detail || data.mensagem || "Não foi possível concluir a operação.");
    return data;
  } catch (error) {
...
function normalizeBook(book) {
  return {
    ...
    title: book.titulo,
    author: book.autor,
    category: book.categoria ?? book.categoria_nome ?? "Sem categoria",
    qty: Number(book.quantidade ?? book.qty ?? 0),
    available: Number(book.disponiveis ?? book.available ?? 0),
    description: book.descricao || book.description || ""
  };
}
...
function renderLoanStudentOptions() {
  ...
  select.innerHTML = `<option value="">Selecione o aluno</option>${students
...
async function loadLoans() {
  const data = await api("/emprestimos");
  loans = data.map((loan) => ({
    ...loan,
    livroId: loan.livroId ?? loan.exemplar_id,
    alunoId: loan.alunoId ?? loan.usuario_id,
    data: loan.data ?? loan.data_emprestimo
  }));
  renderLoans();
}
...
async function handleLogin(event) {
  ...
  try {
    ...
    localStorage.setItem("novarisToken", result.access_token);
...
$("#logoutBtn").onclick = () => {
  ...
  localStorage.removeItem("novarisToken");
  currentUser = null;
...
async function initialize() {
  const storedToken = localStorage.getItem("novarisToken");
