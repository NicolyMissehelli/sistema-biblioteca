    ...
    try:
        if settings.create_initial_admin:
            ...
            if not existing:
                ...
                # Mantém o administrador inicial sincronizado com o .env.
                # Isso evita o problema de o banco continuar com uma senha antiga.
                if (not verify_password(settings.initial_admin_password, existing.senha_hash)
                        or existing.perfil != Perfil.ADMIN
                        or not existing.ativo):
                    existing.senha_hash = hash_password(settings.initial_admin_password)
                    existing.perfil = Perfil.ADMIN
                    existing.ativo = True
                    db.commit()
    finally:
        db.close()
...
@app.post("/login")
def login_frontend(data: LoginRequest, db: Session = Depends(get_db)):
    """Endpoint compatível com o frontend atual (JSON email/senha)."""
    ...
    return {
        "access_token": token,
        "token_type": "bearer",
        "usuario": UsuarioOut.model_validate(user).model_dump(mode="json"),
    }
...
class AlunoCreateCompat(BaseModel):
    nome: str
    email: EmailStr


@app.get("/alunos", response_model=list[UsuarioOut])
def listar_alunos(db: Session = Depends(get_db), _=Depends(current_user)):
    return list(db.scalars(
        select(Usuario).where(Usuario.perfil == Perfil.LEITOR, Usuario.ativo == True).order_by(Usuario.nome)
    ))


@app.post("/alunos", response_model=UsuarioOut, status_code=201)
def criar_aluno(data: AlunoCreateCompat, db: Session = Depends(get_db), _=Depends(require_roles(Perfil.ADMIN, Perfil.BIBLIOTECARIO))):
    if db.scalar(select(Usuario).where(Usuario.email == data.email)):
        raise HTTPException(409, "Email já cadastrado")
    aluno = Usuario(
        nome=data.nome, email=data.email, senha_hash=hash_password("Devops"), perfil=Perfil.LEITOR, ativo=True
    )
    db.add(aluno)
    db.commit()
    db.refresh(aluno)
...
def criar_livro(data: LivroCreate, db: Session = Depends(get_db), _=Depends(require_roles(Perfil.ADMIN, Perfil.BIBLIOTECARIO))):
    if data.categoria_id is not None:
        ...
        categoria = db.scalar(select(Categoria).where(Categoria.nome.ilike(data.categoria.strip())))
        if not categoria:
            categoria = Categoria(nome=data.categoria.strip())
            db.add(categoria)
            db.flush()
    if data.isbn and db.scalar(select(Livro).where(Livro.isbn == data.isbn)):
        raise HTTPException(409, "ISBN já cadastrado")
    livro = Livro(
        titulo=data.titulo, autor=data.autor, isbn=data.isbn, editora=data.editora,
        ano_publicacao=data.ano_publicacao, categoria_id=categoria.id
    )
    db.add(livro)
    db.flush()
    for i in range(data.quantidade):
        db.add(Exemplar(livro_id=livro.id, tombo=f"AUTO-{livro.id}-{i+1}"))
    db.commit()
    db.refresh(livro)
    return livro
...
def criar_emprestimo(
    ...
    target_user_id = data.aluno_id or data.usuario_id or user.id
    ...
    exemplar = db.get(Exemplar, data.exemplar_id) if data.exemplar_id is not None else None
    if exemplar is None and data.livro_id is not None:
        exemplar = db.scalar(select(Exemplar).where(
            Exemplar.livro_id == data.livro_id, Exemplar.status == StatusExemplar.DISPONIVEL
    ...
    db.refresh(emp)
    return {
        **EmprestimoOut.model_validate(emp).model_dump(),
        "livroId": emp.exemplar.livro_id,
        "alunoId": emp.usuario_id,
        "data": datetime.combine(emp.data_emprestimo, datetime.min.time()),
    }
...
def listar_emprestimos(db: Session = Depends(get_db), _=Depends(require_roles(Perfil.ADMIN, Perfil.BIBLIOTECARIO))):
    emprestimos = list(db.scalars(select(Emprestimo).order_by(Emprestimo.data_emprestimo.desc())))
    return [{
        **EmprestimoOut.model_validate(emp).model_dump(),
        "livroId": emp.exemplar.livro_id,
        "alunoId": emp.usuario_id,
